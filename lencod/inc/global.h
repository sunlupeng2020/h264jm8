...
//! Data Partitioning Modes，数据分区方式
typedef enum
{
  PAR_DP_1,   //!< no data partitioning is supported//没有数据分区
  PAR_DP_3,   //!< data partitioning with 3 partitions//数据分为三个区
} PAR_DP_TYPE;
...
typedef enum {
  FRAME_CODING,//帧编码
  FIELD_CODING,//场编码
  ADAPTIVE_CODING//自适应编码
} CodingType;

//! definition of H.264 syntax elements//语法元素
typedef enum {
  SE_HEADER,
  SE_PTYPE,
  SE_MBTYPE,
...
typedef struct
{
  ...
  float snr_va;              //!< Average SNR V(dB) remaining frames
                             ...
                             //! all input parameters，全部输入参数
typedef struct
{
  int ProfileIDC;               //!< profile idc，档次IDC
  int LevelIDC;                 //!< level idc，级别？
...
typedef struct
{
  int number;                  //!< current image number to be encoded，当前要编码的图像号
  int pn;                      //!< picture number，图片编号
  int nb_references;//
  int current_mb_nr;//当前宏块编号？
  int total_number_mb;//总宏块数
  int current_slice_nr;//当前条带编号
  int type;//类型，有哪些？ 指条带类型（Slice类型）如B条带B_SLICE、P_Slice等，参见SliceType结构定义，
  int structure;               //!< picture structure//图像结构，有哪些种？
  int num_reference_frames;    //!< number of reference frames to be used
  ...
  int MbaffFrameFlag;    //!< indicates frame with mb aff coding//表示宏块使用了宏块aff编码？AFF什么意思？
...
void intrapred_luma(int CurrPixX,int CurrPixY, int *left_available, int *up_available, int *all_available);//,int *all_available2为新增，slp,20191006
void init();
...
extern 	char emb_mode;//1左移，得到一个只有1个1的数
extern 	 char emb_info;
*/

char *emb;//嵌入的信息，20190916，slp改
int encoding_num;//目前处理到第几位了，20190916，slp改
int maxencodenum;//要处理的位数，20190916，slp改
int embcodelength;//嵌入的数据有多少字节
int emb01code[200];
int embinfo[10][99][4][4];//帧、宏块、b8\b4是否嵌入信息