...
#ifdef EMBEDDED
  // extern char *emb="ABCDEF";//嵌入的信息，20190916，slp改
  // extern int encoding_num=0;//目前处理到第几位了，20190916，slp改
   //extern int maxencodenum=48;//要处理的位数，20190916，slp改
   //extern int embcodelength;//嵌入的数据有多少字节
   //extern int emb01code[200];
#endif
                           ...
                           int mostProbableMode)
{
  ...
  *nonzero = dct_luma (block_x, block_y, &dummy, 1);
  ...
  if (input->symbol_mode == UVLC)    writeSyntaxElement_Intra4x4PredictionMode(currSE, dataPart);//写入4x4预测信息
  else                               dataPart->writeSyntaxElement (currSE, dataPart);//写入4x4块信息
...
int Mode_Decision_for_4x4IntraBlocks (int  b8,  int  b4,  double  lambda,  int*  min_cost)
{
  ...
  double  rdcost;
  int     block_x     = 8*(b8%2)+4*(b4%2);//求出4x4子块最左侧像素的x坐标
  int     block_y     = 8*(b8/2)+4*(b4/2);//求出4x4子块最上边像素的y坐标
  int     pic_pix_x   = img->pix_x+block_x;
  int     pic_pix_y   = img->pix_y+block_y;
...
//#ifdef EMBEDDED
//	//extern int encoding_num;
//	//int all_available2;//4x4块的top,left,top_left,top_right都可用
// //   char emb_byte;//取一个字节，括号中算出是第几个字节
// //   int emb_bit;//从右往左第几位？
// //   char emb_mode;//1左移，得到一个只有1个1的数
// //   char emb_info;
//	//int block_index=0;//记录输出的4x4块数量
//#endif
  ...
  *min_cost = (1<<20);
  ...
  //对4x4块做帧内预测，计算9种预测值，存放在三维数组mprr[9][4][4]中
  intrapred_luma (pic_pix_x, pic_pix_y, &left_available, &up_available, &all_available);//&all_available2为新增，slp,20191006，20191208去掉了
//#ifdef EMBEDDED
  //if(all_available !=(left_available && up_available))
//	  printf("not equal.\n");
  //if(encoding_num<maxencodenum &&all_available)
  //{ 
	 // //if(encoding_num==17) getchar();
	 ////printf("encoding_num: %d\n",encoding_num);
	 //emb_byte=emb[(encoding_num)/8];//取一个字节，括号中算出是第几个字节
	 //emb_bit=encoding_num%8;//从右往左第几位？
	 //emb_mode=1<<emb_bit;//1左移，得到一个只有1个1的数
	 //emb_info=emb_byte&emb_mode;
	 //if(emb_info)printf("1");	 else printf("0");
  //}
//#endif
  ...
  for (ipmode=0; ipmode<NO_INTRA_PMODE; ipmode++)
  {
	  //if(ipmode!=4)continue;
	   //20191013,改为注释
#ifdef EMBEDDED  //孙陆鹏添加的代码,20190916
  int   m=emb01code[encoding_num];
  if(encoding_num<maxencodenum && ((m &&ipmode%2==0) || (!m)&&(ipmode%2==1 )))
  {//如果all_available，这一块如何处理?
	   //只对all_available的4x4块进行嵌入,
		//不符合要求的预测模式，跳过，不予考虑和计算,1对应1,3,5,7,  0对应0,2,4,6,8
		   continue;
  }
#endif

    available_mode =  (ipmode==DC_PRED) ||
    ...
    if( available_mode)
    {
      ...
      else//input->rdopt为1的情况，一般情况下，该值为1
      {
//#ifdef EMBEDDED  //孙陆鹏添加的代码,20190916
//  
//  if(encoding_num<maxencodenum)
//  {//如果all_available，这一块如何处理?
//	  // if(all_available && (emb_info &&ipmode%2==0 || (!emb_info)&&ipmode%2==1 ))//只对all_available的4x4块进行嵌入
//	//	   continue;
//  if(img->current_mb_nr<2)
//	 printf("img->current_mb_nr:%d,block_x:%d,block_y:%d,pic_block_x:%d,pic_block_y:%d,best_ipmode:%d\n",img->current_mb_nr,block_x,block_y,pic_block_x,pic_block_y,best_ipmode );
//  if(block_index<32){
//	 printf("mb_nr:%d,\tbk_x:%d,bk_y:%d,\tp_bk_x:%d,\tp_bk_y:%d,b_d:%d\n",img->current_mb_nr,block_x,block_y,pic_block_x,pic_block_y,best_ipmode );
//	block_index++;
//  }
//
//	  ++encoding_num;
...
//#endif
        ...
        if ((rdcost = RDCost_for_4x4IntraBlocks (&c_nz, b8, b4, ipmode, lambda, min_rdcost, mostProbableMode)) < min_rdcost)
        {
          //--- set coefficients ---
          for (j=0; j<2; j++)
          for (i=0; i<18;i++)  cofAC4x4[j][i]=img->cofAC[b8][b4][j][i];//为什么是18？

          //--- set reconstruction ---
          for (y=0; y<4; y++)
          for (x=0; x<4; x++)  rec4x4[y][x] = enc_picture->imgY[pic_pix_y+y][pic_pix_x+x];//imgY中保存了重建的图像
 ...
 */
 void encode_one_macroblock ()
 {
   ...
   //===== SET LAGRANGE PARAMETERS =====////阶段1之计算lambda_motion
   if (input->rdopt)
   {
...
//          else cost8x8 += min_rdcost;          
    ...
    if (input->rdopt)
    {
      ...
      for (currMB->c_ipred_mode=DC_PRED_8; currMB->c_ipred_mode<=PLANE_8; currMB->c_ipred_mode++)
      {
        ...
        for (ctr16x16=0, index=0; index<7; index++)//循环得到最佳块模式
        {
          ...
          if (valid[mode])
          {
            ...
            }//endif  if (currMB->c_ipred_mode == DC_PRED_8 ||(IS_INTRA(currMB) ))
          }
    ...
    else////////!(input->rdopt)
    {
      ...
      {
        intrapred_luma_16x16 ();//计算I16MB的4种模式下的预测值，保存在img->mprr_2中
...
//在此处嵌入信息?否定！！！！！20200202下午
//#ifdef EMBEDDED
	//if(EMBEDDED == 1)
	//{
//		if(mode==I4MB)//如果是4x4帧内模式，则嵌入信息
//		{
//			printf("mode=I4MB ");
//				//printf("mode=%d ",mode);
//
