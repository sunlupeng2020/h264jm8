 ...
 ************************************************************************
 */
void intrapred_luma(int img_x,int img_y, int *left_available, int *up_available, int *all_available)//int *all_available2为新增，,int *all_available2slp20191006
{
  int i,j;
  int s0;
  ...
  int ioff = (img_x & 15);
  int joff = (img_y & 15);
  int mb_nr=img->current_mb_nr;//当前宏块索引号
  ...
  pix_c.available = pix_c.available && !(((ioff==4)||(ioff==12)) && ((joff==4)||(joff==12)));//为什么这样算？slp,20191007

  if (input->UseConstrainedIntraPred)
  {
    for (i=0, block_available_left=1; i<4;i++)
      block_available_left  &= pix_a[i].available ? img->intra_block[pix_a[i].mb_addr]: 0;//为什么左边的块需要多次计算？
 ...
 // *all_available2 = *all_available &&block_available_up_right;// if(EMBEDDED)
