 ...
 *  \author
 *   Main contributors (贡献者，投稿者，捐助者。see contributors.h for copyright, address and affiliation（联系，从属关系） details)
...
void embeddedcode()//把嵌入的数据写入到数组中，每个数组元素对应嵌入数据的一位，20191231
{
    char emb_byte;//取一个字节，括号中算出是第几个字节
	char emb_mode;//1左移，得到一个只有1个1的数
	char emb_info;
    int i,j,k,m;
	emb="Hello world!Hello world!Hello world!Hello world!Hello world!Hello world!";//嵌入的信息，20190916，slp改
    encoding_num=0;//目前处理到第几位了，20190916，slp改
    //maxencodenum=96;//要嵌入多少位
	embcodelength=strlen(emb);
	maxencodenum=embcodelength*8;//要嵌入多少位
	for(i=0,k=0;i<embcodelength;i++)
	{
		emb_byte=emb[i];
		for(j=0;j<8;j++,k++)
		{
