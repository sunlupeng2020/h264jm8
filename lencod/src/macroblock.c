...
int write_ipremode(int frame_numm,int mb_nrr,int mbtypee,int blockxx, int blockyy,int modee)
{
	FILE *fp3;
	fp3=fopen("E:\\演示\\鲁棒性算法\\嵌入位置\\文本\\789_basketballpass.txt","at+");
	if(fp3==NULL)
	{
		printf("open file<789_basketballpass.txt.txt> failed!\n");
		return -1;
	}
	fprintf(fp3,"\nf%dm%dt%dx%dy%do%d",frame_numm,mb_nrr,mbtypee,blockxx,blockyy,modee);
 ...
 ************************************************************************
 * \brief
 *    Predict one component of a 4x4 Luma block//预测一个4X4亮度块的一个组成（成分）
 ************************************************************************
 */
void
OneComponentLumaPrediction4x4 (int*   mpred,      //  --> array of prediction values (row by row)//最终预测结果？
                               int    pic_pix_x,  // <--  absolute horizontal coordinate of 4x4 block//水平绝对坐标
                               int    pic_pix_y,  // <--  absolute vertical   coordinate of 4x4 block//垂直绝对坐标
                               int*   mv,         // <--  motion vector//运动矢量
                               int    ref,        // <--  reference frame //参考帧的号
                               StorablePicture **list)
{
  pel_t** ref_pic;//pel_t是byte类型
  int     pix_add = 4;
  ...
  pel_t (*get_pel) (pel_t**, int, int, int, int) = UMVPelY_14;//赋值语句，定义了一个函数指针get_pel，并为它赋值
 ...
 */
void
LumaPrediction4x4 (int  block_x,    // <--  relative horizontal block coordinate of 4x4 block相对水平块地址？0~3吗？
                   int  block_y,    // <--  relative vertical   block coordinate of 4x4 block 相对垂直块地址？0~3吗？
                   int  p_dir,      // <--  prediction direction (0=forward, 1=backward, 2=bidir)前向，后向，双向
                   ...
                   int  fw_ref_idx, // <--  reference frame for forward prediction (-1: Intra4x4 pred. with fw_mode)前向预测参考帧
                   int  bw_ref_idx  )    //后向预测参考帧
{
  static int fw_pred[16];
  static int bw_pred[16];

  int  i, j;
  int  block_x4  = block_x+4;//为什么加4？
  int  block_y4  = block_y+4;//
  int  pic_opix_x = img->opix_x + block_x;//img->opix_x是current original picture pixel horizontal当前原始图像像素水平
  int  pic_opix_y = img->opix_y + block_y;//
  int  bx        = block_x >> 2;//除以2
  int  by        = block_y >> 2;//
  int* fpred     = fw_pred;//
  int* bpred     = bw_pred;//
...
void write_one_macroblock (int eos_bit)
{
  ...
  int b8,b4,k,m,x,y;
  int block_x,block_y;
  int upMode,leftMode,rightMode,bottomMode,currMode;
  int mostProbableMode;
  PixelPos left_block;
  PixelPos top_block;
  PixelPos right_block;
  PixelPos bottom_block;
  PixelPos curr_block;
...
#ifdef EMBEDDED
//嵌入信息  20200202
	if(IS_OLDINTRA(currMB)&& encoding_num<maxencodenum )
	{
		//printf("g%d    ",img->number);
		//printf("g%d b%d",img->number,img->current_mb_nr);
		//for(i=0;i<16;i++)
		//{
		//	printf("g%d    ",currMB->intra_pred_modes[i]);
		//	//currMB->intra_pred_modes
		//}
		//for(b8=0,k=0;b8<4;b8++)
		//	for(b4=0;b4<4;b4++,k++)
		//	{

				  //block_x = 8*(b8%2)+4*(b4%2);//求出4x4子块最左侧像素的x坐标
      //            block_y = 8*(b8/2)+4*(b4/2);//求出4x4子块最上边像素的y坐标
		//		  getLuma4x4Neighbour(img->current_mb_nr, block_x/4, block_y/4, 0,  0, &curr_block);
		...
		//		  getLuma4x4Neighbour(img->current_mb_nr, block_x/4, block_y/4, +1,  0, &right_block);
		//		  getLuma4x4Neighbour(img->current_mb_nr, block_x/4, block_y/4,  0, +1, &bottom_block);
		//		  //if( ! left_block.available && ! top_block.available ||
		//		  //求出left和top子块中预测模式值较小的预测模式值，即MostProbableMode
		//		  currMode =  img->ipredmode[curr_block.pos_x][curr_block.pos_y]; 
		...
		//		  rightMode  = right_block.available ? img->ipredmode[right_block.pos_x ][right_block.pos_y ] : -1;
		//		  bottomMode= bottom_block.available ? img->ipredmode[bottom_block.pos_x][bottom_block.pos_y] : -1;				  
  //                //mostProbableMode  = (upMode < 0 || leftMode < 0) ? DC_PRED : upMode < leftMode ? upMode : leftMode;
		//		  //输出img->ipredMode的内容
		//		  //printf("%d %d %d %d --",upMode,leftMode,rightMode,bottomMode);
		//		  //输出第一个宏块的各子块实际的预测模式
		//		  if(currMB->intra_pred_modes[4*b8+b4]!= -1 && rightMode!= currMode && bottomMode != currMode)//本身的预测模式不是-1，下方和右侧的预测模式和本块不同
		//		  {
		//			  //修改该4x4块的预测模式
		...
		//	}
		//在索引号为3的4x4子块嵌入信息，即b8=0,b4=3



		//if(encoding_num<16)//在所有信息嵌入完之前
		//{
		//	b8=0,b4=3;//在第4块嵌入
		//	block_x = 8*(b8%2)+4*(b4%2);//求出4x4子块最左侧像素的x坐标
  //          block_y = 8*(b8/2)+4*(b4/2);//求出4x4子块最上边像素的y坐标
		//	k=img->ipredmode[b8][b4];//
		//	m=emb01code[encoding_num];
		//	if(k%2 != m)
		//	{
		//		if(k==8) k=k-1;
		//		else k=k+1;
		//		img->ipredmode[b8][b4]=k;
		//		upMode=img->ipredmode[b8][1];
		//		leftMode=img->ipredmode[b8][2];
		//		mostProbableMode = upMode > leftMode ? leftMode : upMode;
		//		i= k == mostProbableMode ? -1 : k > mostProbableMode ? k-1 : k;
		//		currMB->intra_pred_modes[b8*4+b4]=i;//
		//		for(x=0;x<4;x++)
		//			for(y=0;y<4;y++)
		//				img->mpr[block_x+x][block_y+y] = img->mprr[k][y][x];
		//	}
		printf("%d ",emb01code[encoding_num]);
			encoding_num++;
...
int writeCBPandLumaCoeff ()
{
  ...
  {
    //=====   C B P   =====//codeblockpattern残差的编码模式，共六位。前两位是关于色度的，后四位是关于亮度的，反映其中有无非0系数。
    //---------------------
    currSE->value1 = cbp;
    ...
    if (IS_OLDINTRA (currMB) || currMB->mb_type == SI4MB)
    {
	  ...
	  //计划在此处写入嵌入信息，不管其后是如何写入的。2019.12.8,slp
	  //
    }
    else
    {
  ...
  else
  {
    ...
    // AC coeffs
    if (cbp & 15)//表示有亮度系数，4个8*8块中至少一个的交流系数不为0
    {
